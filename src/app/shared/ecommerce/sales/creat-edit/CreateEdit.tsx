...
const CreateEditSales = () => {
  ...
  const [selectedProducts, setSelectedProducts] = useState<{ value: number; label: string; sku: string }[]>([]); // Selected products for react-select
    ...
    const fetchSalesData = async () => {
      ...
      if (isEditing && id) {
        try {
          ...
          // Set selected products for react-select
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Construct full product objects (id and sku)
    ...
    const salesPayload = {
      ...
      Products: transformedProducts, // Use the transformed array here
    };

    console.log('Submitting sales payload:', salesPayload);
    ...
    try {
      if (isEditing) {
        ...
        console.log('Sales entry updated:', salesPayload);
        ...
        console.log('Sales entry created:', salesPayload);
      ...
      <div className="mb-6">
        ...
        <Select
          ...
          options={productsList.map((product) => ({
            ...
            sku: product.sku, // Include sku in the selection
          }))}
